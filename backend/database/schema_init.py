...
def _dedup_nodes(session, label: str, prop: str) -> int:
    """Remove duplicate nodes keeping the first-created one.

    Relationships on deleted duplicates are lost intentionally —
    the ingestion pipeline uses MERGE so re-uploading restores them.
    """
    result = session.run(
        f"""
        MATCH (n:{label})
        WITH n.{prop} AS key, COLLECT(n) AS nodes
        WHERE SIZE(nodes) > 1
    ...
    if deleted:
        logger.warning(
            "Deduplication: removed %d duplicate :%s nodes (property: %s)",
            deleted, label, prop,
        )
    return deleted
...
def _apply_constraints(session) -> None:
    for name, label, prop in _CONSTRAINTS:
        # Remove duplicates first — otherwise CREATE CONSTRAINT will fail.
        _dedup_nodes(session, label, prop)
        cypher = (
        ...
        try:
            ...
            logger.warning(
                "Could not create constraint %s (skipping): %s", name, exc
            )
...
def init_schema() -> None:
    ...
    try:
        ...
        logger.warning(
            "Schema initialisation encountered errors (server will still start): %s", exc
        )
